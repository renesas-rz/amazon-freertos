	...
	switch(secure_boot_state)
	{
		...
		case BOOT_LOADER_STATE_BANK1_UPDATE_LIFECYCLE_ERASE_COMPLETE:
            ...
            do
            {
                ...
                /* verify */
                if( 0 == memcmp(BOOT_LOADER_UPDATE_TEMPORARY_AREA_LOW_ADDRESS, firmware_update_control_block_tmp, SF_SECTOR_SIZE) )
                {
                    /* OK */
                    retry = 0;
    	        	...
    	        	switch(secure_boot_state)
					{
    	        		...
    	        		case BOOT_LOADER_STATE_BANK0_INSTALL_CODE_FLASH_READ_WAIT:
                            ...
                            do
                            {
                                ...
                                /* verify */
                                if( 0 == memcmp(BOOT_LOADER_UPDATE_TEMPORARY_AREA_LOW_ADDRESS, hyper, downloaded_image_size) )
                                {
                                    /* OK */
                                    retry = 0;
...
static void bank_swap_with_software_reset(void)
{
    ...
    uint8_t retry = 3;
    uint32_t secure_boot_state_pre = secure_boot_state;
    ...
    do
    {
        ...
        /* verify */
        if( 0 == memcmp(BOOT_LOADER_UPDATE_EXECUTE_AREA_LOW_ADDRESS, hyper, (image_size + BOOT_LOADER_USER_FIRMWARE_HEADER_LENGTH)) )
        {
            /* OK */
            retry = 0;
            secure_boot_state = secure_boot_state_pre;
            /* temporary area clear */	// RZ/A2M OTA 2020.03.19 //
